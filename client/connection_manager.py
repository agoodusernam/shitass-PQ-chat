...
class ConnectionManager:
    ...
    def connect(self, host: str, port: int) -> bool:
        ...
        try:
            ...
            self._ui.display_error_message("Connection timed out. Please try again.")
            self._close_socket()
            return False
        except ConnectionRefusedError:
            self._ui.display_error_message("Connection refused. Please check the server address and port.")
            self._close_socket()
            return False
        except Exception as e:
            self._ui.display_error_message(f"Failed to connect to server: {e}")
            self._close_socket()
            return False
    ...
    def _oversized(self, message_data: bytes) -> bool:
        ...
        self._ui.display_error_message(
                "Received overly large message without key verification. Dropping. ({len(message_data)} bytes)")
